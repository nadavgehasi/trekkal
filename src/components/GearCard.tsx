...
const GearCard: React.FC = ({}) => {
  ...
  const updateCategory = (updatedCategory: Category) => {
    let filteredCategories = gear.categories.filter(category => {
      return category.id != updatedCategory.id;
    });
    let updatedGear = new Gear({
      name: gear.name,
      categories: [...filteredCategories, updatedCategory],
    });
    setGear(updatedGear);
  };
  ...
  const addCategory = (newCategoryName: string) => {
    ...
    let updatedGear = new Gear({
      name: gear.name,
      categories: [...gear.categories, newCategory],
    });
    setGear(updatedGear);
  };
